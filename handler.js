...
export async function participantsUpdate({ id, participants, action }) {
 ...
 /* conn.sendButtonDoc(id, wm, text, action == 'add' ? 'ᴡᴇʟᴄᴏᴍᴇ' : 'sᴀʏᴏɴᴀʀᴀᴀ', action === 'add' ? '.intro' : 'Aʟᴅɪ X Aɪsʏᴀʜ', fkontak, { contextInfo: { externalAdReply: { showAdAttribution: true,
  ...
  let welcom = 'https://telegra.ph/file/19e30f3feb06cb66d9011.jpg'

  let godbye = 'https://telegra.ph/file/7f511dcd8b06c7f51f8d8.jpg'
  conn.sendButtonImg(id, await(await fetch(action === 'add' ? wel : lea)).buffer(), 'Group Messege', text, action == 'add' ? '𝖲𝖾𝗅𝖺𝗆𝖺𝗍 𝖣𝖺𝗍𝖺𝗇𝗀 😇' : '𝖲𝖺𝗆𝗉𝖺𝗂 𝖩𝗎𝗆𝗉𝖺 👋', action === 'add' ? '.intro' : 'Aʟᴅɪ X Aɪsʏᴀʜ', fkontak, { contextInfo: { externalAdReply: { showAdAttribution: true,
